...
function Brick:hit(unlockPowerUp)

    -- check if it's a brick with lock
    if (not self.lock or unlockPowerUp) then
        -- set the particle system to interpolate between two colors; in this case, we give
        -- it our self.color but with varying alpha; brighter for higher tiers, fading to 0
        -- over the particle's lifetime (the second color)
        local nextColor = self.color
        if (nextColor > 1) then
...
function Brick:render()
    if self.inPlay then
        ...
        local img = 1 + ((self.color - 1) * 4) + self.tier
        if self.lock then
            --fixed img for locked brick
            img = 22
        end
