...
function LevelMaker.createMap(level, lock)
    ...
    if #bricks == 0 then
        ...
        if lock and not lockAdded then
            bricks[math.ceil(#bricks/2)].lock = true
            bricks[math.ceil(#bricks/2)].color = 1
            bricks[math.ceil(#bricks/2)].tier = 0
        end
        return bricks
    end
end